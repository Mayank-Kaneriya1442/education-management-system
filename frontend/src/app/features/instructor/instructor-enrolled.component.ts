    ...
    <div class="page-container">
      <header class="page-header">
        <div class="header-content">
          <h1><span style="margin-right: 12px">🎓</span>Enrolled Students</h1>
      ...
      <div class="list-card">
          ...
          <table>
            ...
            <tbody>
              <tr *ngFor="let e of enrollments()">
                <td class="font-semibold">{{ studentName(e) }}</td>
                <td>{{ studentEmail(e) }}</td>
                <td>{{ studentPhone(e) }}</td>
        ...
        <ng-template #loadingTpl>
          <div class="loading-state">Loading enrollments...</div>
        </ng-template>
      </div>
    </div>
  `,
...
export class InstructorEnrolledComponent {
  ...
  load() {
    ...
    this.error.set(null);
    this.api.instructorCourseEnrollments(this.courseId).subscribe({
      next: (r) => {
  ...
  private student(e: Enrollment): any {
    return (e as any).student;
  }

  studentName(e: Enrollment): string {
    const s = this.student(e);
    if (!s || typeof s === 'string') return '—';
    return String(s.name ?? '—');
  }

  studentEmail(e: Enrollment): string {
    const s = this.student(e);
    if (!s || typeof s === 'string') return '—';
    return String(s.email ?? '—');
  }

  studentPhone(e: Enrollment): string {
    const s = this.student(e);
    if (!s || typeof s === 'string') return '—';
