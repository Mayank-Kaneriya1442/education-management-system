    ...
    <div class="page-container">
      <header class="page-header">
        <div class="header-content">
          <h1><span style="margin-right: 12px">⭐</span>Review Management</h1>
          ...
          <table>
            ...
            <tbody>
              <tr *ngFor="let r of reviews()">
                <td class="font-semibold">{{ r.course?.title ?? '—' }}</td>
                ...
                <td class="text-right">
                  <div class="action-buttons">
                    <button class="btn btn-icon btn-success" (click)="setStatus(r, 'approved')">Approve</button>
                    <button class="btn btn-icon btn-warning" (click)="setStatus(r, 'rejected')">Reject</button>
                    <button class="btn btn-icon btn-danger" (click)="remove(r)">Delete</button>
                  </div>
                </td>
              </tr>
...
export class ReviewManagementComponent {
  ...
  load() {
    ...
    this.api.adminList().subscribe({
      ...
      error: (e) => {
        this.error.set(e?.error?.message ?? 'Failed to load reviews');
        this.loading.set(false);
      }
    });
  }
  ...
  setStatus(review: Review, status: 'approved' | 'rejected') {
    this.api.adminSetStatus(review._id, status).subscribe({
      next: () => this.load(),
  ...
  remove(review: Review) {
    ...
    this.api.adminDelete(review._id).subscribe({
      next: () => this.load(),
      error: (e) => this.error.set(e?.error?.message ?? 'Failed to delete review')
    });
  }
}
